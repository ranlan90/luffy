...
class AlipayTradeView(APIView):
    alipay = AliPay(
        ...
        app_notify_url='http://122.51.145.16:8000/api/v1/trade/alipay/',
        return_url='http://122.51.145.16:8000/api/v1/trade/alipay/',
        app_private_key_path='api/utils/keys/app_private_2048.txt',
        alipay_public_key_path='api/utils/keys/alipay_public_2048.txt',  # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥
        debug=True,  # 默认False,
    )
    ...
    def get(self,request,*args,**kwargs):
       try:
           ...
           status = self.alipay.verify(params, sign)
           print('GET验证', status)
           if status:
               # 获取订单状态，显示给用户
               out_trade_no = params.get("out_trade_no")  # 商户订单号
               redirect_to = "{0}?order_num={1}".format("http://122.51.145.16:8080/order/pay_success", out_trade_no)
    ...
    def post(self, request):
        ...
        if trade_status == "TRADE_SUCCESS":
            ...
            # 修改订单状态
            save_status = self.change_order_status(order_sn, trade_no, gmt_payment, "alipay")
    ...
    def change_order_status(order_num, payment_number, gmt_payment, trade_type):
        """交易成功修改订单相关的状态
        ...
        try:
            ...
            # 修改优惠券状态为已使用
            exist_order.couponrecord_set.update(status_choices=1)
