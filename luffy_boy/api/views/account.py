...
class AccountView(APIView):
    ...
    # 1.course_id不为空查询当前用户拥有未使用的，在有效期的且与当前课程相关的优惠券
    # 2.course_id为空查询当前用户拥有未使用的，在有效期的通用优惠券
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            for course_dict in course_list:
                ...
                print(account_dict["coupon_list"])
            ...
            # 获取通用优惠券,加入redis中
            print('通用')
            print(self.get_coupon_list(request))

            REDIS_CONN.set("global_coupon_%s" % user.pk, json.dumps(self.get_coupon_list(request)))
            REDIS_CONN.set("total_price_%s" % user.pk,sum(price_list))
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            total_price = REDIS_CONN.get("total_price_%s" % request.user.pk)
            res.data = {
