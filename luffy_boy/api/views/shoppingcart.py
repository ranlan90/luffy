...
class  ShoppingCarView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not course_obj:
                raise CommonException(1002, "课程异常!")
            # 2.2 校验价格策略是否能合法
    ...
    def put(self, request):
        ...
        try:
            ...
            course_info = REDIS_CONN.get(shopping_car_key)
            ...
            course_info_dic = course_info["relate_price_policy"][str(price_policy_id)]
            price = course_info_dic.get('price')
            period = course_info_dic.get('valid_period')
            course_info["default_price_policy_id"] = price_policy_id
            course_info["default_price"] = price
            course_info["default_price_period"] = period
            # 4 修改信息后写入redis
