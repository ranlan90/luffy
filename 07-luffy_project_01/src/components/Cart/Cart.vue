<template>
	<div class="shopping-cart-wrap">
		<h3 class="shopping-cart-tit">我的购物车<small>共{{total}}门课程</small></h3>
		<div class="row">
			 <el-table
			     v-loading="loading"
			    ref="multipleTable"
              ...
              <!--{{scope.row.relate_price_policy}}-->
			      	...
			      	<select v-model = 'scope.row.default_price_policy_id' @change="changeHandler(scope.row)">
			      		<option v-for = '(item,index) in scope.row.relate_price_policy' :key = 'index'  :value = 'item.price_policy_id'   >
    ...
    <div style="clear:both;padding: 20px auto;">
...
//
    ...
    changeHandler(row){
      ...
      let params = {
        "course_id":row.id,
        "price_policy_id":row.default_price_policy_id,
      }
      console.log(params)
      this.$http.shoppingcarUpdate(params)
        .then(res=>{
